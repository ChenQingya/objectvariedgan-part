...
"""
class Feature_Similarity_Loss(nn.Module):
    def __init__(self, layers_weights, crop_quarter=False, max_1d_size=100, distance_type=Distance_Type.Cosine_Distance, b=1.0, h=0.1, cuda=True):
        super(Feature_Similarity_Loss, self).__init__()
        listen_list = []
        self.layers_weights = {}
        try:
    ...
    def _random_sampling(tensor, n, indices):
        ...
        indices = Feature_Similarity_Loss._move_to_current_device(indices)
    ...
    def _random_pooling(feats, output_1d_size=100):
        ...
        feats_sample, indices = Feature_Similarity_Loss._random_sampling(feats[0], output_1d_size ** 2, None)
        res = [feats_sample]

        for i in range(1, len(feats)):
            feats_sample, _ = Feature_Similarity_Loss._random_sampling(feats[i], -1, indices)
    ...
    def _create_using_dotP(I_features, T_features):
        assert I_features.size() == T_features.size()
        I_features, T_features = Feature_Similarity_Loss._centered_by_T(I_features, T_features)
        I_features = Feature_Similarity_Loss._normalized_L2_channelwise(I_features)
        T_features = Feature_Similarity_Loss._normalized_L2_channelwise(T_features)
    ...
    def calculate_CX_Loss(self, I_features, T_features):
        I_features = Feature_Similarity_Loss._move_to_current_device(I_features)
        T_features = Feature_Similarity_Loss._move_to_current_device(T_features)
        ...
        if self.distanceType == Distance_Type.L1_Distance:
            raw_distance = Feature_Similarity_Loss._create_using_L1(I_features, T_features)
        elif self.distanceType == Distance_Type.L2_Distance:
            raw_distance = Feature_Similarity_Loss._create_using_L2(I_features, T_features)
        else:
            raw_distance = Feature_Similarity_Loss._create_using_dotP(I_features, T_features)
        ...
        relative_distance = Feature_Similarity_Loss._calculate_relative_distance(raw_distance)
        ...
        # Similarity
        feature_sim = exp_distance / torch.sum(exp_distance, dim=-1, keepdim=True)
        if torch.sum(torch.isnan(feature_sim)) == torch.numel(feature_sim) or torch.sum(
                torch.isinf(feature_sim)) == torch.numel(feature_sim):
            print(feature_sim)
            raise ValueError('NaN or Inf in feature_sim')
        del exp_distance
        max_gt_sim = torch.max(torch.max(feature_sim, dim=1)[0], dim=1)[0]
        del feature_sim
        FS = torch.mean(max_gt_sim, dim=1)
        FS_loss = torch.mean(-torch.log(FS))
        if torch.isnan(FS_loss):
            raise ValueError('NaN in computing FS_loss')
        return FS_loss
...
if __name__ == '__main__':
    ...
    fs_loss = Feature_Similarity_Loss(layers, max_1d_size=64).cuda()
    print('cxloss', fs_loss(I, T))
